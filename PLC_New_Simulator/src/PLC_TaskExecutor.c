...
inline void PLC_TaskExecutor_setContinueWheErrorOccurs(uint8 isContinue)
{
	currStatus.continueWhenErrorOccurs = isContinue;
}
...
inline PLC_Device* PLC_TaskExecutor_getDevice(){
	return &dev;
}

inline uint16 PLC_TaskExecutor_execOneScanTime(uint8 taskIdx, uint32 currPos, EventFlag priority){
	uint32 pos;
	...
	taskIdxToSet = currStatus.currTaskIdx;
	PLC_Register_set(OPERAND_ML_CODE_SD, PLC_SD_REG_RUN_PROG_ID, 0, &taskIdxToSet, TYPE_WORD);
	...
	do{
...

		{
			...
			//DBG_LOGPRINT("[EXEC] [%s - %d] operatorML : %d, SM700 = %d\n",opt->strName, pos, instructionOperator, PLC_Register_getBit(OPERAND_ML_CODE_SM, 700));
#if PLC_LOG_DEF
			...
			//DBG_LOGPRINT("[FIN] [%s - %d] operatorML : %d, SM700 <= %d\n",opt->strName, pos, instructionOperator, PLC_Register_getBit(OPERAND_ML_CODE_SM, 700));
			//ScanTimeLog_print();
#if PLC_LOG_DEF
			if(ret==FALSE)
		...
		if(!ret)
		{
			...
			if(!currStatus.continueWhenErrorOccurs)
				return FALSE;
		}
...
inline void PLC_TaskExecutor_progressTime(){
	int idx;
...
inline uint8 PLC_TaskExecutor_init(){
	...
	currStatus.continueWhenErrorOccurs = TRUE;
...
inline uint8 PLC_TaskExecutor_exec(
		...
		int scanTimeDelay)
{
	...
	for(taskOrder=0; taskOrder<MAX_TASK_COUNT; taskOrder++)
	{
		...
		if( PLC_TaskExecutor_getDevice()->nExecTime == 0
				&& TaskList_getInstance()->list[taskIdx].nLaunchingCondition==CONDITION_ALARM_EXEC)
		{

		}
		else if(TaskList_getInstance()->list[taskIdx].nLaunchingCondition!=CONDITION_ALWAYS_EXEC)
		{
		...
		uint16 val = PLC_TaskExecutor_execOneScanTime(taskIdx, 0, EVENT_NONE);
		if(val!= ML_CODE_END && val!=ML_CODE_FEND)
		{
		...
		PLC_TimeScanManager_notifyExecEnd();
...
inline uint8 PLC_TaskExecutor_execWithDeviceControl_Test(
		int timeCount,
		int scanTimeDelay)
{
...
*/
	...
	uint8 order[] = {0, 1,2,3,4,5,6,7};
	TaskList_setOrder(order);
	PLC_TaskExecutor_setContinueWheErrorOccurs(FALSE);
...
inline uint8 parseToControlBuffer(char* srcLine, PLC_DEVICE_ControlInstruction* control){
...
inline uint8 PLC_TaskExecutor_enterMCLegion(uint8 state)
{
	...
	return STACK_push(&currStatus.stackOnMCState, state)==STACK_TRUE;
}
...
inline uint8 PLC_TaskExecutor_turnOffMCLegion(uint8 state)
{
	...
	//PLC_TaskExecutor_printMCStates();
	return STACK_push(&currStatus.stackOffMCState, state)==STACK_TRUE;
}
