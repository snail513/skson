...
inline uint8 TaskList_MPSBranch(uint8 val)//MPS
{
...
inline uint8 TaskList_MRDBranch(uint8 *val)//MRD
{
...
inline uint8 TaskList_MPPBranch(uint8 *val)//MPP
{
	if(currStackMPSCount == 0)
		return FALSE;
	currStackMPSCount--;
...
inline uint32 binarySearch(uint32 array[], uint32 arraySize, uint32 search)
{
...
inline void TaskList_setOrder(uint8 taskOrder[]){
	memcpy(taskList.taskOrder, taskOrder, MAX_TASK_COUNT);
}

inline uint8 TaskList_getOrder(uint8* taskIdx, uint8 taskOrder)
{
	if(taskOrder>=8)
		return FALSE;
...
inline PLC_IL_Task_List* TaskList_getInstance(){
	return &taskList;
}

inline uint32 TaskList_getCurrTaskEnd(uint32 currPos){
	uint32 prevEndPos = 0;
	int idxTask;
...
inline uint32 TaskList_getTaskSize(uint8 taskIdx)
{
...
inline uint8 TaskList_addTask(uint8 instruction[], uint32 sizeInstruction, LaunchingCondition condition, uint8 taskIdx)
{
	uint8 pos = taskIdx;
	if(taskIdx>=MAX_TASK_COUNT)
		return FALSE;
...
inline uint8 TaskList_AddJumpingPoint(){
...
inline uint8* TaskList_getTaskHead(uint8 taskIdx)
{
	if(taskIdx>PLC_IL_TASK_MAXCOUNT-1)
		return NULL;
	return taskList.list[taskIdx].taskML;
}
inline uint8 TaskList_getTaskCMDValue(uint32 pos){
...
inline uint8 TaskList_getTaskCMDIndexValue(uint8 taskIdx, uint32 pos){
	PLC_IL_Task *task= &(taskList.list[taskIdx]);
	{
		uint32 arrayPos = binarySearch(task->cmdPosList, task->nCurrCMDCount, pos);
		if(arrayPos==0xFFFFFFFF)
...
inline uint8 TaskList_setTaskCMDValue(uint32 pos, uint8 bitVal){
...
inline uint8 Task_loadTasksFromBuffers(uint8 buffer[], int bufSize, uint8 taskIdx, LaunchingCondition condition){
	//DBG_LOGPRINT("\n[LINE]%d\n", __LINE__);
	int pos = 0;
	//int startingPoint = 0;
	...
	while(pos<bufSize)
	{
		...
		{
			pos += OPND_DICT_ByteSize(&buffer[pos]);
			/*if(buffer[pos]==OPERAND_ML_CODE_STR)
			{
				pos+= 1+getOperandStrLength(buffer, (uint32*)&pos);//type[1] str-length[length+\n]
			}
			/*
			else if(buffer[pos]==OPERAND_ML_CODE_N)
				pos+= operandLength*2;//type[1] nibbleCount[2] regType[1] regAddr[2]
			else if(buffer[pos]==OPERAND_ML_CODE_Z)
				pos+= operandLength*2;//type[1] zAddr[2] regType[1] regAddr[2]
			else if(buffer[pos]==OPERAND_ML_CODE_WB)
				pos+= operandLength*2;//type[1] bitAddr[2] regType[1] regAddr[2]
				...
				pos+= 9;//type[1] val[8]
			else if(buffer[pos]==OPERAND_ML_CODE_J)
			{
				pos+= operandLength*2;//type[1] devAddr[2] regType[1] regAddr[2]
			}
			else if(buffer[pos]==OPERAND_ML_CODE_U)
			{
				pos+=5;//type[1] devAddr[2] regAddr[2]
			}
			else
			{
				//pos += 3;//type[1] regAddr[2]
...
inline uint8 TaskList_addLungHeadPos(uint8 taskIdx,uint16 pos){
...
inline uint8 TaskList_changeLaunchingCondition(uint8 taskIdx, LaunchingCondition condition)
{
	if(taskList.list[taskIdx].nCurrStepCount>=PLC_STEP_MAX_COUNT)
	{
		DBG_LOGPRINT("\t[ERROR]overflow (STEP count >= %d)while %s. ",PLC_STEP_MAX_COUNT,__FUNCTION__ );
...
inline void TaskList_resetCMDTask(uint8 taskIdx)
{
	taskList.list[taskIdx].nCurrCMDCount = 0;
}

inline uint8 TaskList_addCMDHeadPos(uint8 taskIdx, uint16 pos){
	//printf("taskIdx : %d, pos : %d\n",taskIdx, pos);
