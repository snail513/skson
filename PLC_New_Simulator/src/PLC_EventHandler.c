...
extern uint16 PLC_TaskExecutor_execOneScanTime(uint8 taskIdx, uint32 currPos, EventFlag priority);
...
inline uint8 PLC_EventHandler_callFunction(uint32 pointLabel)
{
	...
	if(PLC_TaskExecutor_execOneScanTime(interruptionTaskIdx, interruptionTaskPos, 0)!=ML_CODE_RET)
	{
