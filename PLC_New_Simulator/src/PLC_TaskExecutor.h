...
void PLC_TaskExecutor_setContinueWheErrorOccurs(uint8 isContinue);
...
uint16 PLC_TaskExecutor_execOneScanTime(uint8 taskIdx, uint32 currPos, EventFlag priority);
...
uint8 PLC_TaskExecutor_exec(
		int scanTimeDelay);
/****************************/
void PLC_TaskExecutor_setInitTaskTimeInfo(timespec_t rst);
void PLC_TaskExecutor_setTaskTimeInfo(uint8 taskOrder, uint8 taskIdx, timespec_t rst);
void PLC_TaskExecutor_setTimeInfo(int SM_ADDR, timespec_t rst);
